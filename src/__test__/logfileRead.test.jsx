...
describe("<LogFileRead>", () => {
  it("should containing five box with their initial content", async () => {
    ...
    let [info, warning, error, total, header] =container.getElementsByClassName("MuiCardMedia-root");

    expect(container.getElementsByClassName("MuiCardMedia-root").length).toBe(5);
    ...
    expect(header.textContent).toBe("Reading log file every 1000 Mili-second ...");
  });

  it("should skleton shows at the begining", async ()=>{
       const { container } = renderComponent();
       await waitFor(()=> expect(container.getElementsByClassName("MuiSkeleton-root").length).not.toBe(0));
  })
});
